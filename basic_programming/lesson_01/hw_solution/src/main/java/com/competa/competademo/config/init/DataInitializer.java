...
public class DataInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        userDto.setNickName("user_user");
        adminUserDto.setNickName("admin_user");
        createRoles();

        User createdUser = saveUser(userDto);
        User adminUser = saveUser(adminUserDto);

        if (adminUser != null) {
            userService.addUserRole(adminUser.getId(), ROLE_ADMIN);
            userService.updateUserStatus(adminUser.getId(), UserStatus.CONFIRMED);
        }
        if (createdUser != null) {
            userService.updateUserStatus(createdUser.getId(), UserStatus.CONFIRMED);
        }
    }

    private User saveUser(final CreateUserDto userDto) {
